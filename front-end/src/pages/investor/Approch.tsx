...
type Profile = {
  ...
  hasActiveSubscription: boolean; // New property from backend
};
...
const InvApproch: React.FC = () => {
  ...
  const [investorHasPaidSubscription, setInvestorHasPaidSubscription] = useState(false); // ✅ New state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setInvestorHasPaidSubscription(res.data.investorHasPaidSubscription);
      } catch (error) {
        Swal.fire({
          title: "Error",
          text: "Failed to fetch idealogists or subscription.",
          icon: "error",
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {profiles.map((profile) => {
          ...
          // ✅ New locking logic: lock if investor has no subscription OR idea holder has no subscription
          const isLocked = !investorHasPaidSubscription || !profile.hasActiveSubscription;
