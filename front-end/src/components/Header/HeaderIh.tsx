...
const HeaderIh: React.FC = () => {
  ...
  const AvatarWithFirstLetter = () => {
    ...
    return (
      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center bg-gray-700 text-white font-semibold shadow-sm text-sm sm:text-base">
          ...
          {ideaHolderMenu.map((item) => (
            ...
            >
              ...
              {item.name === "Notifications" && inviteCount > 0 && (
                <span className="absolute -top-2 -right-3 bg-red-500 text-white text-xs w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full shadow">
                  {inviteCount}
                </span>
              )}
            </Link>
