...
export default function PaymentPage() {
  const [amount, setAmount] = useState(1499.0);
  ...
  // Input handlers and validators
  const handleCardInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    // Card number: allow only numbers and add spaces after every 4 digits
    if (name === "number") {
  ...
  const handleBlur = (field: string) => {
    let valid = true;
    if (field === "number") {
      valid = /^\d{4} \d{4} \d{4} \d{4}$/.test(card.number);
    } else if (field === "name") {
      valid = card.name.trim().length > 2;
    } else if (field === "expiry") {
      valid = /^\d{2}\/\d{2}$/.test(card.expiry);
    } else if (field === "cvv") {
      valid = card.cvv.length === 3 || card.cvv.length === 4;
    } else if (field === "upi") {
      valid = /^[\w.\-]+@[\w.\-]+$/.test(upi);
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-2">
      <div className="w-full max-w-xl bg-white shadow-2xl rounded-3xl p-6 md:p-9">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="text-sm text-gray-500 font-medium">Amount to Pay</div>
            <div className="text-2xl font-extrabold text-gray-900 flex items-center">
              <span>₹</span>
              <span className="ml-1">{amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
          <svg width={44} height={44} viewBox="0 0 44 44"><circle cx={22} cy={22} r={22} fill="#f3f4f6" /></svg>
        </div>
        ...
        <div>
            ...
            <>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  />
                  {errors["name"] && (
                    <p className="text-red-500 text-xs mt-1">Enter a valid name</p>
                  )}
                </div>
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      className={`w-full bg-gray-100 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 transition text-gray-900 tracking-widest ${
                        errors["number"] ? "border-2 border-red-500" : "border border-gray-200"
                      }`}
                      placeholder="0000 0000 0000 0000"
                    />
                    {errors["number"] && (
                      <p className="text-red-500 text-xs mt-1">Enter a valid card number</p>
                    )}
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full bg-gray-100 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 transition text-gray-900 text-center ${
                        errors["expiry"] ? "border-2 border-red-500" : "border border-gray-200"
                      }`}
                      placeholder="MM/YY"
                    />
                    {errors["expiry"] && (
                      <p className="text-red-500 text-xs mt-1">Invalid date</p>
                    )}
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      maxLength={4}
                      className={`w-full bg-gray-100 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 transition text-gray-900 text-center ${
                        errors["cvv"] ? "border-2 border-red-500" : "border border-gray-200"
                      }`}
                      placeholder="123"
                    />
                    {errors["cvv"] && (
              ...
              <div>
                ...
                <div className="relative w-full">
                  ...
                  <span className="absolute right-3 top-3 text-gray-400 pointer-events-none">
                    <svg width="18" height="18" viewBox="0 0 20 20"><path d="M7 7l3 3 3-3" stroke="currentColor" strokeWidth={2} fill="none" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </span>
                </div>
              </div>
                ...
                {BANKS.map((b) => (
                  <div
                    ...
                    className={`p-2 rounded-lg border transition flex items-center text-xs font-semibold ${
                      bank === b.name ? "border-blue-600 text-blue-700 bg-blue-100" : "border-gray-200 bg-gray-50"
                    }`}
                  >
                    {b.icon}
          ...
          {method === "upi" && (
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium mb-1">UPI ID or Wallet Number</label>
                <input
                  name="upi"
                  type="text"
                  autoComplete="off"
                  ...
                  className={`w-full bg-gray-100 px-3 py-2 rounded-lg focus:outline-none focus:ring-2 transition text-gray-900 ${
                    errors["upi"] ? "border-2 border-red-500" : "border border-gray-200"
                  }`}
                  placeholder="eg: mobile@upi or 98xxxxxx98"
                />
                {errors["upi"] && (
                  <p className="text-red-500 text-xs mt-1">Enter valid UPI ID / wallet</p>
                )}
              </div>
        ...
        {/* Demo note */}
        <div className="mt-3 text-xs text-gray-400 text-center">Demo UI for Payment Checkout – React + Tailwind CSS</div>
      </div>
    </div>
  );
}
...
function CardMockup({
  ...
  // Card flip animation with Tailwind
  return (
    <div className="perspective-850 w-72 h-44">
      <div className={`relative w-72 h-44 transition-transform duration-700`} style={{
        transformStyle: 'preserve-3d',
        transform: showBack ? 'rotateY(180deg)' : 'rotateY(0deg)'
      }}>
        {/* FRONT */}
        <div className="absolute w-full h-full bg-gradient-to-br from-blue-50 via-blue-100 to-blue-400 rounded-2xl shadow-xl flex flex-col justify-between p-5"
          style={{ backfaceVisibility: 'hidden' }}>
          <div className="flex justify-between items-center">
            <span className="font-bold text-gray-700 text-sm">Credit / Debit Card</span>
            <span className="flex items-center space-x-2">{CARD_ICONS.map(i => i)}</span>
          </div>
          <div>
            <div className="text-lg font-mono tracking-widest text-gray-900">{number || "0000 0000 0000 0000"}</div>
            <div className="flex justify-between items-center mt-3">
              <div>
                <div className="text-xs uppercase text-gray-500">Cardholder</div>
                <div className="text-sm font-semibold text-gray-800">{name || "FULL NAME"}</div>
              </div>
              <div>
                <div className="text-xs uppercase text-gray-500">Expiry</div>
                <div className="text-sm font-semibold text-gray-800">{expiry || "MM/YY"}</div>
              </div>
            </div>
          </div>
        </div>
        {/* BACK */}
        <div className="absolute w-full h-full bg-gradient-to-br from-blue-700 via-blue-400 to-blue-300 rounded-2xl shadow-xl px-5 pt-6 pb-4"
          style={{
...
function VisaIcon() {
  return (
    <svg width="28" height="15" viewBox="0 0 28 15" aria-label="Visa">
      ...
      <text x="6" y="10" fill="white" fontSize="8" fontWeight="bold">VISA</text>
    </svg>
  );
}
function MasterCardIcon() {
  return (
    <svg width="28" height="15" viewBox="0 0 28 15" aria-label="MasterCard">
      ...
      <text x="7" y="12" fill="#6d28d9" fontSize="6" fontWeight="bold">MC</text>
    </svg>
  );
}
function RuPayIcon() {
  return (
    <svg width="28" height="15" viewBox="0 0 28 15" aria-label="RuPay">
      ...
      <text x="4" y="10" fill="#0d9488" fontSize="8" fontWeight="bold">RuPay</text>
    </svg>
  );
}
...
function GooglePayIcon() {
  return (
    <svg width="23" height="23" viewBox="0 0 23 23">
      ...
      <g>
        <path d="M8.5 12.2V10.8h4.6c0.05.25.1.55.1.95 0 2.85-2.2 4.1-4.7 4.1A5 5 0 018 13.55l1.3-.8c.25.45.7.95 1.95.95 1.25 0 2.05-.75 2.05-1.85 0-.15-.02-.3-.06-.45z"
          fill="#34a853"/>
        <path d="M8 13.55A4.99 4.99 0 017 11.5c0-.65.1-1.35.28-1.95l1.32.8a2.67 2.67 0 00-.09 1.15c0 .4.13.81.38 1.19z"
          fill="#fbbc05"/>
        <path d="M12.5 10.8a2.13 2.13 0 00-.08-.6V9.7h-2.87v1.66H8.5V10.8h4z"
          fill="#4285f4"/>
        <path d="M10.37 8.8H12.5V7.34c-.44-.14-.93-.22-1.48-.22C9.08 7.12 7.5 8.52 7.05 10.07l1.32.8a2.78 2.78 0 012-2.07z"
          fill="#ea4335"/>
      </g>
    </svg>
  );
...
function PhonePeIcon() {
  return (
    <svg width="23" height="23" viewBox="0 0 23 23">
      <circle cx="11.5" cy="11.5" r="11.5" fill="#fff" />
      <text x="7" y="16" fill="#8b5cf6" fontWeight="bold" fontSize="10">पे</text>
    </svg>
  );
}
function PaytmIcon() {
  return (
    <svg width="23" height="23" viewBox="0 0 23 23">
      <circle cx="11.5" cy="11.5" r="11.5" fill="#fff" />
      <text x="5" y="16" fill="#0ea5e9" fontWeight="bold" fontSize="10">Paytm</text>
    </svg>
  );
}
